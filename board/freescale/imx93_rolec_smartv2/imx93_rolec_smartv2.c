...
int board_usb_init(int index, enum usb_init_type init)
{
	...
	if (!tcpc_present) {
		/*
		 * Boards without PTN5110/TCPC use fixed USB roles from DT.
		 * For gadget/fastboot flows, allow controller init to proceed.
		 */
...
{
#ifdef CONFIG_USB_TCPC
	tcpc_present = fdt_node_offset_by_compatible(gd->fdt_blob, -1,
						     "nxp,ptn5110") >= 0;
	if (tcpc_present)
		setup_typec();
#endif
...
int board_late_init(void)
{
	...
	env_set("board_name", "11X11_ROLEC_SMARTV2");
